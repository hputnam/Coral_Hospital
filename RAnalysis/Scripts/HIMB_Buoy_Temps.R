...
plot(Temp_2013$Date.Time, Temp_2013$WTMP, cex=0.1, col="pink", xlab="Date", ylab="Temperature °C", ylim=c(20,31))
points(Temp_2014$Date.Time, Temp_2014$WTMP, cex=0.1, col="lightsalmon")
points(Temp_2015$Date.Time, Temp_2015$WTMP, cex=0.1, col="yellow")
points(Temp_2016$Date.Time, Temp_2016$WTMP, cex=0.1, col="light green")
points(Temp_2017$Date.Time, Temp_2017$WTMP, cex=0.1, col="light blue")
points(Temp_2018$Date.Time, Temp_2018$WTMP, cex=0.1, col="plum")
points(Temp_2019$Date.Time, Temp_2019$WTMP, cex=0.1, col="black")
legend("topleft", legend=c("2013", "2014", "2015", "2016", "2017", "2018", "2019"),
       col=c("pink", "lightsalmon", "yellow", "light green", "light blue", "plum", "black"), lty=1, cex=0.8)
dev.off()


Temp_2013 <- Temp_2013[21491:nrow(Temp_2019),]
Temp_2014 <- Temp_2014[21491:nrow(Temp_2019),]
Temp_2015 <- Temp_2015[21491:nrow(Temp_2019),]
Temp_2016 <- Temp_2016[21491:nrow(Temp_2019),]
Temp_2017 <- Temp_2017[21491:nrow(Temp_2019),]
Temp_2018 <- Temp_2018[21491:nrow(Temp_2019),]
Temp_2019 <- Temp_2019[21491:nrow(Temp_2019),]

pdf("Output/KbayTemps_Apr_Sept.pdf")
plot(Temp_2013$Date.Time, Temp_2013$WTMP, cex=0.1, col="pink", xlab="Date", ylab="Temperature °C", ylim=c(22,31))
points(Temp_2014$Date.Time, Temp_2014$WTMP, cex=0.1, col="lightsalmon")
points(Temp_2015$Date.Time, Temp_2015$WTMP, cex=0.1, col="yellow")
points(Temp_2016$Date.Time, Temp_2016$WTMP, cex=0.1, col="light green")
points(Temp_2017$Date.Time, Temp_2017$WTMP, cex=0.1, col="light blue")
